...
static bool WorldToScreenPx(const vec3_t world, int& sx, int& sy)
{
    float nx, ny, nz;
...
static bool ComputeScreenBoxFromBBox(const vec3_t org, const vec3_t mins, const vec3_t maxs, int pad, int& x1, int& y1, int& x2, int& y2)
{
    vec3_t c[8] = {
...
static bool HasLineOfSight(const vec3_t src, const vec3_t dst)
{
    pmtrace_t tr;
...
public:
    ...
    void Redraw(float time, int intermission)
    {
        ...
        bool losOnly   = clesplos.value > 0.0f;
        bool showLabels= clesplabels.value > 0.0f;
        bool onlySci   = clespscientists.value > 0.0f;
